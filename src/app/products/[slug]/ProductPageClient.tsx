...
import { FREE_SHIPPING_THRESHOLD, DELIVERY_COPY } from '@/config/commerce'
...
function useViewContentPixel(product: Product) {
  useEffect(() => {
...
function PDPBreadcrumb({ product }: { product: Product }) {
  return (
    <div className="max-w-[1320px] mx-auto px-6 lg:px-12 pt-24 pb-4">
      <nav className="flex items-center gap-2 text-[11px] tracking-widest uppercase text-text-medium flex-wrap">
        ...
        {product.subcategory && (
          <>
            <ChevronRight size={10} className="text-text-medium/40 flex-shrink-0" />
            <span className="text-text-medium/60 capitalize">
              {product.subcategory.replace(/-/g, ' ')}
            </span>
          </>
        )}
        <ChevronRight size={10} className="text-text-medium/40 flex-shrink-0" />
        <span className="text-forest-green font-light truncate max-w-[180px]">
          {product.name}
        </span>
      </nav>
    </div>
    ...
    <div className="lg:w-[42%] xl:w-[40%] lg:sticky lg:top-24 lg:self-start flex flex-col">
      ...
      <div className="mb-5">
        <div className="flex items-center gap-3 mb-3">
          <span className="text-[11px] tracking-widest uppercase text-text-medium font-light">
            Qty
          </span>
      ...
      <div className="flex items-center gap-2.5 py-3.5 border-y border-cream mb-7">
        <svg
          ...
          stroke="currentColor"
          strokeWidth="1.5"
          className="text-forest-green flex-shrink-0"
        >
...
function PDPAccordions({
  sections,
  openSection,
  toggle,
}: {
  sections: Array<{ key: string; title: string; content: string; note?: string }>
...
function PDPEditorialHero({ product, heroImgs }: { product: Product; heroImgs: string[] }) {
  return (
...
function PDPHowToUseSection({ product }: { product: Product }) {
  const steps = [
    {
      step: '01',
      title: 'Apply',
      ...
      <div className="max-w-[1320px] mx-auto px-6 lg:px-12">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-0 md:gap-px bg-[#ddd8ce]">
          {steps.map(s => (
            <div
              key={s.step}
              className="bg-cream p-10 lg:p-14 text-center flex flex-col items-center"
            >
              ...
              <h3 className="font-serif text-xl font-light text-forest-green mb-4">
                {s.title}
              </h3>
              <p className="text-[13px] text-text-medium font-light leading-relaxed max-w-[260px]">
                {s.text}
              </p>
            </div>
          ))}
...
function PDPIngredientSpotlights({
  keyIngredients,
}: {
  keyIngredients: Array<{ name: string; image: string; source: string; benefit: string }>
}) {
      ...
      <div className="max-w-[1320px] mx-auto px-6 lg:px-12">
        <div className="text-center mb-16">
          ...
          <p className="text-[13px] text-text-medium font-light mt-4 max-w-lg mx-auto leading-relaxed">
            We source every ingredient with intention. Below is what makes this formula
            exceptional — and why.
          </p>
        </div>
              ...
              >
                ...
                <div
                  className={`absolute inset-0 ${
                    i % 2 === 1 ? 'bg-gradient-to-l' : 'bg-gradient-to-r'
                  } from-forest-green/20 to-transparent`}
                />
              </div>
              <div
                className={`bg-[#F2EDE5] flex flex-col justify-center px-10 lg:px-16 py-14 ${
...
function PDPSustainabilitySection({ product }: { product: Product }) {
  return (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 lg:gap-24 items-center">
          ...
          <div className="grid grid-cols-2 gap-5">
            {[
              ...
              <div
                key={stat.label}
                className="border border-cream/10 p-8 hover:border-gold/30 transition-colors duration-500"
              >
                <p className="font-serif text-4xl font-light text-gold mb-2">{stat.value}</p>
                <p className="text-cream/50 text-[11px] tracking-wide uppercase font-light leading-snug">
                  {stat.label}
                </p>
              </div>
            ))}
...
function PDPSecondaryHero({ product, heroImgs }: { product: Product; heroImgs: string[] }) {
  return (
...
export default function ProductPageClient({
  ...
  const sections: Array<{ key: string; title: string; content: string; note?: string }> = [
    ...
    },
    {
      ...
      title: 'How To Use',
      content:
        'Apply a generous amount to damp skin. Massage in circular motions to work into a rich lather. Rinse thoroughly with warm water. For best results, use daily as part of your morning or evening ritual. Follow with a matching body lotion to seal in moisture and fragrance.',
    },
    {
      key: 'ingredients',
      title: "What's Inside",
  ...
  return (
    <div className="bg-warm-white">
      ...
      <PDPEditorialHero product={product} heroImgs={heroImgs} />
      <PDPHowToUseSection product={product} />
