...
export default function ProductPage({ params }: { params: { slug: string } }) {
    ...
    return (
      <div className="min-h-screen bg-warm-white flex items-center justify-center pt-20">
        <div className="text-center">
          <div className="w-12 h-12 border border-forest-green/20 border-t-forest-green rounded-full animate-spin mx-auto mb-4" />
          <p className="text-xs tracking-widest uppercase text-text-medium">Loading...</p>
        </div>
      </div>
    )
  }
  ...
  if (!product) {
    return (
      <div className="min-h-screen bg-warm-white flex items-center justify-center pt-20">
        <div className="text-center">
          <p className="font-serif text-3xl font-light text-forest-green mb-4">Product not found</p>
          <Link href="/collections/bath-body" className="btn-primary">Browse Collection</Link>
        </div>
      </div>
    )
  }
            ...
            <div className="relative aspect-square overflow-hidden bg-cream mb-4">
              ...
              {product.isBestseller && (
                <div className="absolute top-6 left-6">
                  <span className="bg-burgundy text-cream text-xs px-4 py-2 tracking-widest uppercase">
                    Bestseller
                  </span>
                </div>
              )}
              {product.isNew && (
                <div className="absolute top-6 right-6">
                  <span className="bg-forest-green text-cream text-xs px-4 py-2 tracking-widest uppercase">
                    New
                  </span>
                </div>
              )}
